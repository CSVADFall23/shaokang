...
class NotePlayer{
    ...
    async load(url){
        ...
        await handler.loadMidi(url).then(()=>{
           ...
           this.trackSettings.push({instrument: INSTRUMENTS.hasOwnProperty(track.instrument.toLowerCase()) ? INSTRUMENTS[track.instrument.toLowerCase()]:"acoustic_grand_piano", reverb:0.3,shift:0, minPitch:127,maxPitch:0});
